...
2. **SQLite 데이터베이스 읽기 전용(Read-Only)**: `data.db` 파일에 접근할 때는 절대로 데이터를 덮어쓰거나 수정해서는 안 됩니다. **반드시 파이썬의 `sqlite3` 연결 시 `mode=ro` 속성과 `uri=True` 옵션을 사용**하여 읽기 권한만 갖도록 코드를 작성하십시오.
